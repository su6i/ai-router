...
### Fixed
- **(T-970, in progress) Read-side divergence detection for the code index.** `ingested_files` can hold rows for a repo whose `code_chunks` rows are gone (they were deleted by something outside `ingest()`'s own guarded paths) — the incremental skip-check is keyed on `content_hash` alone, so once this happens the repo silently never gets re-indexed by any future sweep. New `src/code_index.py` helpers `_code_chunk_and_ingested_counts()` and `_diverged_repo_names()` compute, per repo, `(code_chunks rows, ingested_files rows)` for the `code` collection without ever using SQL `LIKE` on a repo-name-prefixed key (a repo name containing `_` is a wildcard to `LIKE` — same reasoning as the existing `--force` GC path); `repo_status()` exposes `{repo: {chunks, files, diverged}}` for callers, returning `{}` (never a false "clean") when Postgres is unreachable. **Not yet wired into anything** — `sweep()` self-heal, the `rag_ingest.py --status`/exit-status wiring, the per-repo concurrency lock, and tests are still pending; tracked in the T-970 work order.
