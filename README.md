...
2. **Preamble Injection**: A constant 5-line preamble of strict reading rules is injected at the very start of every prompt. Placing it *before* the variable file content maximizes API provider prefix-cache hits.
3. **Template Rules**: The `AGENTS-context-discipline.md` template defines the full ruleset (e.g., read a file once, use `grep -n`, batch related reads, don't paste large files back).
