...
`*.py .sh .js .jsx .ts .tsx .go .rs` files are chunked and embedded with the
same local e5-small model, then stored in pgvector next to a static call
graph. `.py`/`.sh` get real function/class/method boundaries via tree-sitter
AST; the rest get a coarser generic whole-file chunker (no tree-sitter
grammar for those languages ships in this repo — adding one is a new
dependency, out of scope here) so a JS/TS-only repo is still searchable
instead of contributing 0 chunks. The index spans **every repo under
`$HOME/@-github/`** (T-953), not just this checkout — `code_chunks.repo`
(the swept repo's own directory name) discriminates. Full design and honest
economics: [`docs/CODE-RAG.md`](docs/CODE-RAG.md).
...
1. **Repo Map**: A compact (`< 4500` chars) repository map of top-level symbols is automatically generated by `src/repo_map.py` and prepended to all worker/agent prompts.
